  ...
  _initExciterDeck() {
    ...
    chordBtns.forEach((btn) => {
      ...
      btn.addEventListener('click', (e) => {
        // Suppress duplicate click event generated by browser immediately after pointerup
        if (btn._lastPointerTime && (Date.now() - btn._lastPointerTime) < 500) {
          e.preventDefault();
          e.stopPropagation();
          return;
        }

        // Programmatic click fallback (e.g. automated test suites)
        if (!this.isPowered) return;
